...
import {
  ...
  increaseQuantity,
  decreaseQuantity,
} from "./cartSlice"; // Import necessary actions
import PropTypes from "prop-types";

function CartItem({ item }) {
  const dispatch = useDispatch();
  const product = useSelector((state) =>
    state.products.allProducts.find((p) => p.id === item.id)
  ...
  const handleDecrease = () => {
    if (item.quantity > 1) {
      ...
      // Show minimalist warning before removing item
  ...
  return (
    <div className="flex justify-between items-center p-4 border-b border-gray-300">
      <img src={item.image} alt={item.title} className="w-16 object-cover" />
